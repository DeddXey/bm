...
struct Tim {
    ...
    struct CR2 {
        ...
        constexpr static uint8_t CCUS[]{2, 1};    //< Capture/compare control update selection
        constexpr static uint8_t CCPC[]{0, 1};    //< Capture/compare preloaded control
    };
    ...
    struct DIER {
        ...
        constexpr static uint8_t CC4DE[]{12, 1};    //< Capture/Compare 4 DMA request enable
        constexpr static uint8_t CC3DE[]{11, 1};    //< Capture/Compare 3 DMA request enable
        constexpr static uint8_t CC2DE[]{10, 1};    //< Capture/Compare 2 DMA request enable
        constexpr static uint8_t CC1DE[]{9, 1};    //< Capture/Compare 1 DMA request enable
        constexpr static uint8_t UDE[]{8, 1};    //< Update DMA request enable
        constexpr static uint8_t TIE[]{6, 1};    //< Trigger interrupt enable
        constexpr static uint8_t CC4IE[]{4, 1};    //< Capture/Compare 4 interrupt enable
        constexpr static uint8_t CC3IE[]{3, 1};    //< Capture/Compare 3 interrupt enable
        constexpr static uint8_t CC2IE[]{2, 1};    //< Capture/Compare 2 interrupt enable
        constexpr static uint8_t CC1IE[]{1, 1};    //< Capture/Compare 1 interrupt enable
    ...
    struct SR {
        constexpr static uint8_t CC4OF[]{12, 1};    //< Capture/Compare 4 overcapture flag
        constexpr static uint8_t CC3OF[]{11, 1};    //< Capture/Compare 3 overcapture flag
        constexpr static uint8_t CC2OF[]{10, 1};    //< Capture/compare 2 overcapture flag
        constexpr static uint8_t CC1OF[]{9, 1};    //< Capture/Compare 1 overcapture flag
        ...
        constexpr static uint8_t CC4IF[]{4, 1};    //< Capture/Compare 4 interrupt flag
        constexpr static uint8_t CC3IF[]{3, 1};    //< Capture/Compare 3 interrupt flag
        constexpr static uint8_t CC2IF[]{2, 1};    //< Capture/Compare 2 interrupt flag
        constexpr static uint8_t CC1IF[]{1, 1};    //< Capture/compare 1 interrupt flag
    ...
    struct EGR {
        ...
        constexpr static uint8_t COMG[]{5, 1};    //< Capture/Compare control update generation
    ...
    struct CCMR1_OUT {
        ...
        constexpr static uint8_t OC2PE[]{11, 1};    //< Output Compare 2 preload enable
        ...
        constexpr static uint8_t OC1PE[]{3, 1};    //< Output Compare 1 preload enable
    ...
    struct CCMR2_OUT {
        ...
        constexpr static uint8_t OC4PE[]{11, 1};    //< Output compare 4 preload enable
        ...
        constexpr static uint8_t OC3PE[]{3, 1};    //< Output compare 3 preload enable
    ...
    struct CCER {
        constexpr static uint8_t CC4P[]{13, 1};    //< Capture/Compare 3 output Polarity
        constexpr static uint8_t CC4E[]{12, 1};    //< Capture/Compare 4 output enable
        constexpr static uint8_t CC3NP[]{11, 1};    //< Capture/Compare 3 output Polarity
        constexpr static uint8_t CC3NE[]{10, 1};    //< Capture/Compare 3 complementary output enable
        constexpr static uint8_t CC3P[]{9, 1};    //< Capture/Compare 3 output Polarity
        constexpr static uint8_t CC3E[]{8, 1};    //< Capture/Compare 3 output enable
        constexpr static uint8_t CC2NP[]{7, 1};    //< Capture/Compare 2 output Polarity
        constexpr static uint8_t CC2NE[]{6, 1};    //< Capture/Compare 2 complementary output enable
        constexpr static uint8_t CC2P[]{5, 1};    //< Capture/Compare 2 output Polarity
        constexpr static uint8_t CC2E[]{4, 1};    //< Capture/Compare 2 output enable
        constexpr static uint8_t CC1NP[]{3, 1};    //< Capture/Compare 1 output Polarity
        constexpr static uint8_t CC1NE[]{2, 1};    //< Capture/Compare 1 complementary output enable
        constexpr static uint8_t CC1P[]{1, 1};    //< Capture/Compare 1 output Polarity
        constexpr static uint8_t CC1E[]{0, 1};    //< Capture/Compare 1 output enable
    };

//    struct CNT {
//        constexpr static uint8_t CNT[]{ 0, 16 }; 	//< counter value
//    };

//    struct PSC {
//        constexpr static uint8_t PSC[]{ 0, 16 }; 	//< Prescaler value
//    };

//    struct ARR {
//        constexpr static uint8_t ARR[]{ 0, 16 }; 	//< Auto-reload value
//    };

//    struct RCR {
//        constexpr static uint8_t REP[]{ 0, 8 }; 	//< Repetition counter value
//    };

//    struct CCR1 {
//        constexpr static uint8_t CCR1[]{ 0, 16 }; 	//< Capture/Compare 1 value
//    };

//    struct CCR2 {
//        constexpr static uint8_t CCR2[]{ 0, 16 }; 	//< Capture/Compare 2 value
//    };

//    struct CCR3 {
//        constexpr static uint8_t CCR3[]{ 0, 16 }; 	//< Capture/Compare value
//    };

//    struct CCR4 {
//        constexpr static uint8_t CCR4[]{ 0, 16 }; 	//< Capture/Compare value
//    };
    ...
    struct BDTR {
        ...
        constexpr static uint8_t OSSR[]{11, 1};    //< Off-state selection for Run mode
        constexpr static uint8_t OSSI[]{10, 1};    //< Off-state selection for Idle mode
    ...
    struct DMAR {
        constexpr static uint8_t DMAB[]{0, 16};    //< DMA register for burst accesses
    };
    ...
    static void setTRGO(Timer::Trgo trgo) {
        tl::setRegister(rg()->CR2,
                        CR2::MMS, static_cast<uint8_t>(trgo));
    }

    static void setCounter(uint16_t counter) { rg()->CNT.CNTK = counter; }

    static void setArr(uint16_t arr) { rg()->ARR.ARR = arr; }
    ...
    static void enableUpdateInterrupt(bool en) {
        tl::setRegister(rg()->DIER,
                        DIER::UIE, en);
    }

    static void enableCc1Interrupt(bool en) {
        tl::setRegister(rg()->DIER,
                        DIER::CC1IE, en);
    }

    static void enableCc2Interrupt(bool en) {
        tl::setRegister(rg()->DIER,
                        DIER::CC2IE, en);
    }

    static void enableCc3Interrupt(bool en) {
        tl::setRegister(rg()->DIER,
                        DIER::CC3IE, en);
    }

    static void enableCc4Interrupt(bool en) {
        tl::setRegister(rg()->DIER,
                        DIER::CC4IE, en);
    }

    static void clearCc1InterruptFlag() {
        tl::setRegister(rg()->SR,
                        SR::CC1IF, 0);
    }


    static void enableUpdateDma(bool en) { rg()->DIER.UDE = en; }

    static void clearUpdateInterruptFlag() { rg()->SR.UIF = 0; }

    static bool updateInterruptFlag() { return (rg()->SR.UIF == 1); }


    static void enableMainOutput([[maybe_unused]] const bool en) {
        switch (num) {
    ...
    static void selectTi1(Timer::Ti1Selection selection) { rg()->CR2.TI1S = selection; }
    ...
    static uint32_t getCounter() { return rg()->CNT.CNTK; }
    ...
    ///------------------------------------------------------------------------
    static void setCcInputFilter(uint8_t channel, Timer::InputCaptureFilter filter) {
        uint32_t index = ((channel - 1) >> 1) & 0b01;
        uint32_t pos = (channel - 1) & 0b01;
    ...
    ///------------------------------------------------------------------------
    static void enableCcOutputN(uint8_t channel, bool en) {
