...
struct Scb
{
  ...
  struct ICSR
  {
    ...
    constexpr static uint8_t PENDSTCLR[]{
      25,
      1
    }; //< SysTick exception clear-pending bit
    constexpr static uint8_t PENDSTSET[]{
      26,
      1
  ...
  struct VTOR
  {
    constexpr static uint8_t TBLOFF[]{ 9,
                                       21 }; //< Vector table base offset field
  };
  ...
  struct CCR
  {
    constexpr static uint8_t NONBASETHRDENA[]{
      0,
      1
  ...
  struct SHPR2
  {
    constexpr static uint8_t PRI_11[]{ 24,
                                       8 }; //< Priority of system handler 11
  };

  struct SHPR3
  {
    constexpr static uint8_t PRI_14[]{ 16,
                                       8 }; //< Priority of system handler 14
    constexpr static uint8_t PRI_15[]{ 24,
                                       8 }; //< Priority of system handler 15
  };

  struct SHCSR
  {
    constexpr static uint8_t MEMFAULTACT[]{
      0,
      1
    }; //< Memory management fault exception active bit
    constexpr static uint8_t BUSFAULTACT[]{
      1,
      1
    }; //< Bus fault exception active bit
    constexpr static uint8_t USGFAULTACT[]{
      3,
      1
    ...
    constexpr static uint8_t PENDSVACT[]{ 10,
                                          1 }; //< PendSV exception active bit
    constexpr static uint8_t SYSTICKACT[]{ 11,
                                           1 }; //< SysTick exception active bit
    constexpr static uint8_t USGFAULTPENDED[]{
      12,
      1
    }; //< Usage fault exception pending bit
    constexpr static uint8_t MEMFAULTPENDED[]{
      13,
      1
    }; //< Memory management fault exception pending bit
    constexpr static uint8_t BUSFAULTPENDED[]{
      14,
      1
    }; //< Bus fault exception pending bit
    constexpr static uint8_t SVCALLPENDED[]{ 15, 1 }; //< SVC call pending bit
    constexpr static uint8_t MEMFAULTENA[]{
      16,
      1
  ...
  {
    constexpr static uint8_t IACCVIOL[]{
      0,
      1
    }; //< Instruction access violation flag
    constexpr static uint8_t DACCVIOL[]{
      1,
      1
    }; //< Instruction access violation flag
    constexpr static uint8_t MUNSTKERR[]{
      3,
      1
    }; //< Memory manager fault on unstacking for a return from exception
    constexpr static uint8_t MSTKERR[]{
      4,
      1
    }; //< Memory manager fault on stacking for exception entry.
    constexpr static uint8_t MLSPERR[]{ 5, 1 }; //< MLSPERR
    constexpr static uint8_t MMARVALID[]{
      7,
      1
    ...
    constexpr static uint8_t IMPRECISERR[]{ 10,
                                            1 }; //< Imprecise data bus error
    constexpr static uint8_t UNSTKERR[]{
      11,
      1
    }; //< Bus fault on unstacking for a return from exception
    constexpr static uint8_t STKERR[]{
      12,
      1
    }; //< Bus fault on stacking for exception entry
    constexpr static uint8_t LSPERR[]{
      13,
      1
    }; //< Bus fault on floating-point lazy state preservation
    constexpr static uint8_t BFARVALID[]{
      15,
      1
    }; //< Bus Fault Address Register (BFAR) valid flag
    constexpr static uint8_t UNDEFINSTR[]{
      16,
      1
    ...
    constexpr static uint8_t UNALIGNED[]{ 24,
                                          1 }; //< Unaligned access usage fault
    constexpr static uint8_t DIVBYZERO[]{ 25,
                                          1 }; //< Divide by zero usage fault
  };
  ...
  static uint32_t getBusFaultAddress()
  {
    return (rg()->BFAR);
  }

  static void
  enableSeparatedFaults(const bool bus, const bool memory, const bool usage)
  {
