...
/**
 * PinAltFunction enumeration. Defines all possible arguments for Alternate()
 * function.
 */
enum PinAltFunction
{
  ...
  afTamper =
    0x00,         ///< TAMPER (TAMPER_1 and TAMPER_2) Alternate Function mapping
...
struct Gpio
{
  ...
  ///
  template<typename T>
  static void setAltFunc(const int val, const T pos)
  {
    uint32_t mask  = tl::setBitGroup(4, (1 << 4) - 1, pos);
    uint32_t value = tl::setBitGroup(4, val, pos);
  ...
  static void setAltFunc(const int val, const T pos, const Args... args)
  {
    uint32_t mask  = tl::setBitGroup(4, (1 << 4) - 1, pos);
    uint32_t value = tl::setBitGroup(4, val, pos);
  ...
  static void configPins(const PinMode       mode,
  ...
  ///
  template<typename... Args>
  static void setSpeed(const PinSpeed speed, const Args... args)
  {
    uint32_t mask  = tl::setBitGroup(2, (1 << 2) - 1, args...);
    uint32_t value = tl::setBitGroup(2, speed, args...);
  ...
  ///
  template<typename... Args>
  static void setMode(const PinMode mode, const Args... args)
  {
    uint32_t mask  = tl::setBitGroup(2, (1 << 2) - 1, args...);
    uint32_t value = tl::setBitGroup(2, mode, args...);
  ...
  ///
  template<typename... Args>
  static void setOType(const PinOutputType type, const Args... args)
  {
    uint32_t mask  = tl::setBitGroup(1, (1 << 1) - 1, args...);
    uint32_t value = tl::setBitGroup(1, type, args...);
  ...
  ///
  template<typename... Args>
  static void setPuPd(PullUpMode mode, const Args... args)
  {
    uint32_t mask  = tl::setBitGroup(2, (1 << 2) - 1, args...);
    uint32_t value = tl::setBitGroup(2, mode, args...);
  ...
  ///
  template<typename... Args>
  static void cpl(const Args... args)
  {
    uint32_t value = tl::setBitGroup(1, 1, args...);
