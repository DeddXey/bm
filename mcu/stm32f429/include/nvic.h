...
struct Nvic
{
  ...
  enum IrqType {
    ...
    itTim1TrgComTim11 = 26, /*!< TIM1 Trigger and Commutation Interrupt and
    ...
    itTim8TrgComTim14 = 45, /*!< TIM8 Trigger and Commutation Interrupt and
  ...
  {
    Scb::rg()->AIRCR = (Scb::aircrKey << 16)
                       + ((priorityGrouping & 0xf) << Scb::aircrPriGroupPos);
  }

  static uint32_t getPriorityGrouping()
  {
    return ((Scb::rg()->AIRCR >> Scb::aircrPriGroupPos) & 0x0f);
  }

  static void enableIrq(Nvic::IrqType interrupt_number)
  {
    rg()->ISER[interrupt_number >> 5] = (1 << (interrupt_number & 0x1f));
  }
  static void disableIrq(Nvic::IrqType interrupt_number)
  {
    rg()->ICER[interrupt_number >> 5] = (1 << (interrupt_number & 0x1f));
  }
  static bool getPendingIrq(Nvic::IrqType interrupt_number)
  {
    return (rg()->ISPR[interrupt_number >> 5]
            & (1 << (interrupt_number & 0x1f)))
           != 0;
  }
  static void setPendingIrq(Nvic::IrqType interrupt_number)
  {
    rg()->ISPR[interrupt_number >> 5] = (1 << (interrupt_number & 0x1f));
  }
  static void clearPendingIrq(Nvic::IrqType interrupt_number)
  {
    rg()->ICPR[interrupt_number >> 5] = (1 << (interrupt_number & 0x1f));
  }
  static bool getActive(Nvic::IrqType interrupt_number)
  {
    return (rg()->IABR[interrupt_number >> 5]
            & (1 << (interrupt_number & 0x1f)))
           != 0;
  }
  static void setPriority(Nvic::IrqType interrupt_number, uint32_t priority)
  {
    if (interrupt_number < 0) {
      Scb::rg()->SHPR[(interrupt_number & 0xf) - 4] =
        ((priority << (8 - nvicPrioBits)) & 0xff);
    }
    else {
      rg()->IP[interrupt_number] = ((priority << (8 - nvicPrioBits)) & 0xff);
    }
  }

  static uint32_t getPriority(Nvic::IrqType interrupt_number)
  {
    if (interrupt_number < 0) {
      return (Scb::rg()->SHPR[(interrupt_number & 0xf) - 4]
              >> (8 - nvicPrioBits));
    }
    else {
      return (rg()->IP[interrupt_number] >> (8 - nvicPrioBits));
    }
  }

  [[noreturn]] static void systemReset()
  {
    cpu::dsb();
