...
struct DmaStream {
    ...
    static void clockEnable(const bool val) { Controller::clockEnable(val); }

    static void enable(bool val) { rg()->CR.EN = val; }
    static void intDirectModeError(bool en) { rg()->CR.DMEIE = en; }
    static void intTransferError(bool en) { rg()->CR.TEIE = en; }
    static void intHalfTransfer(bool en) { rg()->CR.HTIE = en; }
    static void intTransferComplete(bool en) { rg()->CR.TCIE = en; }
    static void peripheralFlowController(bool en) { rg()->CR.PFCTRL = en; }
    static void setDmaDirection(Dma::DmaDirection dir) { rg()->CR.DIR = dir; }
    static void circularMode(bool en) { rg()->CR.CIRC = en; }
    static void peripheralIncrement(bool en) { rg()->CR.PINC = en; }
    static void memoryIncrement(bool en) { rg()->CR.MINC = en; }
    static void setPeripheralDataSize(Dma::DataSize sz) { rg()->CR.PSIZE = sz; }
    static void setMemoryDataSize(Dma::DataSize sz) { rg()->CR.MSIZE = sz; }
    static void setPeripheralIncrementOffsetTo4(bool en) { rg()->CR.PINCOS = en; }
    static void setPriorityLevel(Dma::PriorityLevel pl) { rg()->CR.PL = pl; }
    static void doubleBufferMode(bool en) { rg()->CR.DBM = en; }
    static void setCurrentTarget(Dma::CurrentTarget target) { rg()->CR.CT = target; }
    static void setMemoryBurst(Dma::Burst burst) { rg()->CR.MBURST = burst; }
    static void setPeripheralBurst(Dma::Burst burst) { rg()->CR.PBURST = burst; }
    static void setChannel(uint8_t channel) { rg()->CR.CHSEL = channel; }

    static void setMemoryAddress0(uint32_t address) { rg()->M0AR = address; }
    static void setMemoryAddress1(uint32_t address) { rg()->M1AR = address; }
    static void setPeripheralAddress(uint32_t address) { rg()->PAR = address; }
    static void setTransferSize(uint32_t sz) { rg()->NDTR = sz; }
    static void setFifoMode(const bool en) { rg()->FCR.DMDIS = en; }
    static void setFifoThreshold(const Dma::FifoThreshold ft) { rg()->FCR.FTH = ft; }

    static bool isTransferComplete() { return DmaController<dma>::getTransferCompleteFlag(stream); }
    static void clearAllFlags() { DmaController<dma>::clearAllFlags(stream); }
    ...
    {
      con.debug() << "DMA " << Use::dec << Use::w0 << dma << " stream "
                   << stream << ": " << Use::endl
                   << Use::w8 << Use::hex;
